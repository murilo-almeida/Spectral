...
void DG_Prob::DG_EI_IG(const double sigma,
                       const double sigma1,
                       const double beta, int & count,
                       int * Ti,int * Tj, double * Tx,
                       double * B)
{
for(int bcount=0; bcount < NL;++bcount){
  const EDGE local_border = border[bcount];
    double aux,aux1,aux2,aux3;
    int m,lado;
    MyElem e[2];

    int tipo=local_border.tipo;

    e[0]=el[local_border.elemento[0]];
    const int ndim=e[0].show_ptr_stdel(0)->ndim_val();// numero de dimensooes
    const int qmax=el[0].show_ptr_stdel(0)->qborder_val();
    double w[qmax];

// calculo dos pesos de Gauss multiplicados pelo Jacobiano nas facetas
    double length;
    if(ndim==1){
      length=1.0;
    }
    else if(ndim==2){ // facets tem uma dimensao
      double Jac[qmax],Dtemp[MAXQ][MAXQ];
      Gauss_Jacobi_parameters(qmax,0.0,0.0,Jac,w,Dtemp);
      length = local_border.comprimento;
    ...
    n_e[i]=local_border.normal[i];
  }

    lado=0;
    e[0].perm_val(perm[0]);
    a[0]=local_border.num_local[0];
 ...
 //   fprintf(fout1,"%4d %4d %4d ",index,local_border.Na,local_border.Nb);index++;
 //   int e0=local_border.elemento[0];
 //   int s0=local_border.sinal[0];
 //   fprintf(fout1,"   %4d %4d %5d ",e0,a[0],s0);
 //   if(tipo==2){
 //     e0=local_border.elemento[1];
 //     s0=local_border.sinal[1];
 //     a[1]=local_border.num_local[1];
 ...
 //   printf("Inside EdgeIntegrals: tipo = %2d \n",tipo);
  ...
  // **************************************************************************
    ...
    if(tipo==2){ // tipo==2  Interior Edges
      lado=1;
      a[lado]=local_border.num_local[lado];
      e[lado]=el[local_border.elemento[lado]];
...
//	    int lp_on_border=e[iEl].show_ptr_stdel(pres)->is_on_border(lp,a[iEl],pos);
  ...
  // **************************************************************************
    ...
    pdir=local_border.pdir;
    if(tipo == -1){
      //printf("Inside EdgeIntegrals: tipo = -1\n");
      sdir=local_border.sdir;
