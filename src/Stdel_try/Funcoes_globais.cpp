...
int teste_aresta(int p,int n0,int n1,int& sign,
		  ...
		  int& NG,int& NL, std::vector<ARESTA>& aresta,
		  int Ng[],// candidato a sair
		  int nel,int naresta,
...
// ****************************************************************************
{
  ...
  for (int i=0; i<NL && flag==0; i++){
    //printf("NL = %d i=%d\n",NL,i);
    if(aresta[i].Na==n0 && aresta[i].Nb==n1){
      //ng= Ng[i];//candidato a sair
      flag=1;
      aresta[i].num_elem += 1;
      aresta[i].elemento[1]=nel;
      aresta[i].num_local[1]=naresta;
      aresta[i].sinal[1]=1;
      aresta[i].tipo=2;// aresta interior
      edge_num = i;
    }
    else if(aresta[i].Na==n1 && aresta[i].Nb==n0){// aresta ja percorrida em sentido oposto
      //ng= Ng[i];//candidato a sair
      flag=1;
      sign=-1;
      aresta[i].num_elem += 1;
  ...
  if(flag==0){// aresta nova
    ...
    // calculo da normal
    temp_aresta.num_elem = 1;
    temp_aresta.Na=n0;
    temp_aresta.Nb=n1;
    temp_aresta.elemento[0]=nel;
    temp_aresta.num_local[0]=naresta;
    temp_aresta.sinal[0]=1;
    temp_aresta.elemento[1]=0;
    temp_aresta.num_local[1]=0;
    temp_aresta.sinal[1]=123;
    temp_aresta.tipo=0;// aresta de contorno (inicialmente todas sao no-flow)
		...
		if(sinal_normal!=0) { // gira vetor (x,y) de sinal_normal*pi/2 no sentido anti-horario
			...
			temp_aresta.modulo=modulo;
			temp_aresta.normal[0] = -sinal_normal*y/modulo;
			temp_aresta.normal[1] =  sinal_normal*x/modulo;
		}
			...
			temp_aresta.modulo=modulo;
			temp_aresta.normal[0] = x/modulo;
			temp_aresta.normal[1] = y/modulo;
			temp_aresta.normal[2] = z/modulo;
		}
    aresta.push_back(temp_aresta); // insere copia de temp_aresta no final de aresta
